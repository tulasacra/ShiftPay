...
- A code with no scheme prefix (a bare recipient address) is matched against the recipient formats of the supported networks. One plausible network is used automatically; when several fit (e.g. a `3…` P2SH address, valid on both Bitcoin and Litecoin) the picker offers only those.
